...
function categoryRank(category: ItemCategory): number {
...
function orderSkus(
  ...
  switch (strategy) {
    ...
    case "dynamicSlotting":
      ...
      return [...catalog].sort(
        (a, b) =>
          b.demandWeight / Math.max(0.1, b.volume) -
...
function orderLocations(
  ...
  const byDistance = [...warehouse.storageLocations].sort(
    (a, b) => a.distanceToNearestStation - b.distanceToNearestStation,
  );
  ...
  const buckets = new Map<number, StorageLocation[]>();
  ...
  const interleaved: StorageLocation[] = [];
