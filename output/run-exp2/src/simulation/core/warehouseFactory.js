...
import { cellId, inBounds, manhattanDistance, positionKey, } from "../../utils/grid";
...
export function buildWarehouse(config, layoutRng, stationRng, skuCatalogRng) {
    ...
    const elevatorZones = createElevatorZones(config.warehouse.width, config.warehouse.height, levels);
    const pickingStations = createPickingStations(config.warehouse.width, config.warehouse.height, config.warehouse.pickingStationCount, config.warehouse.pickingStationOrientation, stationRng, config.warehouse.pickingStationLaneCount, config.warehouse.customPickingStations, new Set(elevatorZones.flatMap((elevator) => elevator.cells.map(positionKey))));
    const chargingStations = createChargingStations(config.warehouse.width, config.warehouse.height, config.warehouse.chargingStationCount, config.warehouse.pickingStationOrientation, stationRng, new Set(pickingStations.flatMap((station) => station.accessPositions.map(positionKey))));
    ...
    for (const connector of interMatrixConnectors) {
        for (const position of connector.cells) {
            const cell = cells.find((candidate) => candidate.x === position.x && candidate.y === position.y);
            if (cell?.type === "empty") {
                setCell(cells, position, { type: "rail" });
            }
        }
    }
    const { racks, storageLocations } = createRackLayout(cells, config, pickingStations, subMatrices);
    const { rails, switches } = createRailNetwork(cells, config, pickingStations, chargingStations, elevatorZones);
    ...
    return applyStorageStrategy(warehouse, catalog, config.storage.strategy, layoutRng);
}
...
function createPickingStations(width, height, count, orientation, rng, laneCount = 2, customPositions = [], blockedPositions = new Set()) {
    ...
    for (let index = stations.length; index < count; index += 1) {
        ...
        const aisleIndex = Math.min(aisleColumns.length - 1, Math.max(0, Math.round(((stationIndex + 1) * (aisleColumns.length - 1)) / (count + 1)) + rng.int(-1, 1)));
        const autoAccessPosition = orientation === "width"
            ? {
            ...
            : {
                x: 1,
                y: Math.min(height - 2, Math.max(1, Math.round(((stationIndex + 1) * height) / (count + 1)) +
                    rng.int(-1, 1))),
            };
        const accessPosition = findNearestAvailableStationCell(autoAccessPosition, width, height, used, blockedPositions);
        const position = orientation === "width"
        ...
        for (let lane = 0; lane < lanes; lane += 1) {
            ...
            const candidate = orientation === "width"
            ...
            if (candidate.x >= xMin &&
                ...
                !accessPositions.some((existing) => existing.x === candidate.x && existing.y === candidate.y)) {
...
function createStation(index, position, accessPosition, accessPositions) {
    return {
        id: `STATION_${index + 1}`,
...
function findNearestAvailableStationCell(preferred, width, height, used, blocked) {
    const isOpen = (position) => inBounds(position, width, height) &&
...
function createChargingStations(width, height, count, pickingStationOrientation, rng, excludedPositions = new Set()) {
...
function createWarehouseLevels(count) {
...
function createSubMatrices(config) {
...
function createInterMatrixConnectors(subMatrices) {
    const connectors = [];
    const byCoordinate = new Map(subMatrices.map((matrix) => [`${matrix.row}:${matrix.column}`, matrix]));
...
function computeAisleLayout(width) {
    const aisleColumns = [];
    const rackColumns = [];
    ...
    let cursor = minX;
    while (cursor <= maxX) {
        for (let offset = 0; offset < RACK_COLUMNS_PER_ELEVATOR_AISLE && cursor <= maxX; offset += 1) {
...
export function getElevatorAisleCountForWidth(width) {
...
function computeCorridorRows(height, count) {
...
function createRackLayout(cells, config, pickingStations, subMatrices) {
    const racks = [];
    const storageLocations = [];
    const reserved = new Set(cells
        .filter((cell) => cell.type !== "empty")
        .map((cell) => positionKey(cell)));
    const corridorRows = new Set(computeCorridorRows(config.warehouse.height, Math.round(config.warehouse.crossAisleSpacing ?? 3)));
    ...
    for (let y = 2; y < config.warehouse.height - 2; y += 1) {
        ...
        for (const x of rackColumns) {
            ...
            const position = { x, y };
            if (reserved.has(positionKey(position)) ||
                isSubMatrixBoundary(position, config, subMatrices)) {
                continue;
            }
            const accessPosition = findAccessPosition(position, config.warehouse.width, config.warehouse.height, reserved);
            if (!accessPosition) {
                continue;
            }
            const rackId = `RACK_${racks.length + 1}`;
            const locationIds = Array.from({ length: config.warehouse.levelCount }, (_, level) => `LOC_${racks.length + 1}_${level + 1}`);
            const distanceToNearestStation = Math.min(...pickingStations.map((station) => manhattanDistance(accessPosition, station.accessPosition)));
            racks.push({
                id: rackId,
                position,
                width: 1,
...
function getRackTarget(cells, config, reserved, corridorRows, subMatrices) {
...
function isSubMatrixBoundary(position, config, subMatrices) {
    ...
    return subMatrices.some((matrix) => {
        ...
        const nearVerticalBoundary = matrix.column < config.warehouse.subMatrixColumns - 1 &&
            Math.abs(position.x - rightEdge) < corridorWidth;
        const nearHorizontalBoundary = matrix.row < config.warehouse.subMatrixRows - 1 &&
            Math.abs(position.y - bottomEdge) < corridorWidth;
        return nearVerticalBoundary || nearHorizontalBoundary;
    });
}
function countSubMatrixRacks(subMatrices, racks) {
    return subMatrices.map((matrix) => ({
        ...matrix,
        rackCount: racks.filter((rack) => rack.position.x >= matrix.origin.x &&
            rack.position.x < matrix.origin.x + matrix.width &&
            rack.position.y >= matrix.origin.y &&
            rack.position.y < matrix.origin.y + matrix.height).length,
    }));
}
function findAccessPosition(rackPosition, width, height, reserved) {
    const candidates = [
    ...
    return candidates.find((candidate) => inBounds(candidate, width, height) && !reserved.has(positionKey(candidate)));
}
function setCell(cells, position, patch) {
    const cell = cells.find((candidate) => candidate.x === position.x && candidate.y === position.y);
    if (cell) {
        Object.assign(cell, patch);
    }
}
function createRailNetwork(cells, config, pickingStations, chargingStations, elevatorZones) {
    const rails = [];
    ...
    for (const rail of rails) {
        for (const position of rail.cells) {
            ...
            const connectedRailIds = rails
                .filter((candidate) => candidate.id !== rail.id &&
                candidate.cells.some((candidatePosition) => candidatePosition.x === position.x &&
...
function isRailCandidate(position, cellByKey) {
    const cell = cellByKey.get(positionKey(position));
