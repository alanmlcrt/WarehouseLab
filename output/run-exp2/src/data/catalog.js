...
export function createSkuCatalog(skuCount, rng) {
    return Array.from({ length: skuCount }, (_, index) => {
        const rank = index / Math.max(1, skuCount - 1);
        const category = rank < 0.2
            ? "fast-moving"
            : rank < 0.55
                ? "medium-moving"
                : "slow-moving";
        const categoryWeight = category === "fast-moving" ? 100 : category === "medium-moving" ? 35 : 8;
        ...
        return {
            ...
            handlingTime: (category === "fast-moving" ? 2 : category === "medium-moving" ? 3 : 4) +
                handlingJitter,
...
export function getDemandWeight(sku, catalog, pattern) {
    if (pattern === "uniform") {
        return 1;
    }
